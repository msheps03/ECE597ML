...
def train_one_vs_all(X, y, num_classes, lambda_val):
    ...
    m, n = X.shape
    weight_vectors = np.zeros((num_classes, n))
    ...
    for i in range(n):
        ...
        for j in range(num_classes):
            ...
            if j == 0:
                weight_temp[j] = weight[j] - alpha*derive(X, y, j, weight)
            else:
                weight_temp[j] = weight[j]*(1-alpha*lambda_val) - alpha*derive(X, y, j, weight)
